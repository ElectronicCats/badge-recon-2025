...
bool DisplayController::initialize(uint16_t width, uint16_t height, uint8_t address) {
  _display = &display;
...
void DisplayController::showWelcomeScreen() {
  ...
  _display->println(F("Recon Badge 2025"));
  _display->println(F("by Electronic Cats"));
  _display->display();
}
